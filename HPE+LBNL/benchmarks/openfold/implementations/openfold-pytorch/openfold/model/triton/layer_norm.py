...
# TODO(@davidli): Find a more elegant approach to cache tuned results.
    ...
    def forward(ctx, inputs, normalized_shape, weight, bias, eps=1e-05):
        ...
        if inputs.is_contiguous():
            _layer_norm_forward[grid](
                x_ptr=inputs, w_ptr=weight, b_ptr=bias, eps=eps,
                x_invstd_ptr=x_invstd, x_mean_ptr=x_mean, y_ptr=y,
                M=M, N=N,
            )
        else:
            ...
            _layer_norm_forward_strided[grid](
                ...
                x_invstd_ptr=x_invstd, x_mean_ptr=x_mean, y_ptr=y,
                M=M, N=N,
                D0=D0, D1=D1, D2=D2, D3=D3, S0=S0, S1=S1, S2=S2, S3=S3,
            )
    ...
    def backward(ctx, d_y):
        ...
        if inputs.is_contiguous():
            _layer_norm_backward_dx[grid](
                dy_ptr=d_y, x_ptr=inputs, w_ptr=weight, x_invstd_ptr=x_invstd, x_mean_ptr=x_mean,
                dx_ptr=d_inputs,
                M=M, N=N,
            )
        else:
        ...
        if inputs.is_contiguous():
            _layer_norm_backward_dw_db_partial[grid](
                dy_ptr=d_y, x_ptr=inputs, x_invstd_ptr=x_invstd, x_mean_ptr=x_mean,
                dw_partial_buf_ptr=dw_partial_buf, db_partial_buf_ptr=db_partial_buf,
                M=M, N=N,
                BUF_N_STRIDE=M_BUFSIZE,
            )
            M_PARTIAL_REDUCE = _layer_norm_backward_dw_db_partial.best_config.kwargs["M_PARTIAL_REDUCE"]
        else:
            _layer_norm_backward_dw_db_partial_strided[grid](
                dy_ptr=d_y, x_ptr=inputs, x_invstd_ptr=x_invstd, x_mean_ptr=x_mean,
                dw_partial_buf_ptr=dw_partial_buf, db_partial_buf_ptr=db_partial_buf,
                M=M, N=N,
                BUF_N_STRIDE=M_BUFSIZE, D0=D0, D1=D1, D2=D2, D3=D3, S0=S0, S1=S1, S2=S2, S3=S3,
            )
            M_PARTIAL_REDUCE = _layer_norm_backward_dw_db_partial_strided.best_config.kwargs["M_PARTIAL_REDUCE"]
        ...
        _layer_norm_backward_buf_reduce[grid](
            ...
            output_ptr=d_weight,
            N=N, M=M_BUFSIZE,
            N_STRIDE=dw_partial_buf.stride(0), M_STRIDE=dw_partial_buf.stride(1),
            num_warps=1,
        )
        _layer_norm_backward_buf_reduce[grid](
            ...
            output_ptr=d_bias,
            N=N, M=M_BUFSIZE,
            N_STRIDE=db_partial_buf.stride(0), M_STRIDE=db_partial_buf.stride(1),
            num_warps=1,
        )
