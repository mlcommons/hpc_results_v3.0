...
# Currently this is necessary, otherwise the SWA modules in /workspace will be imported.
sys.path.insert(0, str(Path(os.path.realpath(__file__)).parents[1]))

import pytest
import torch

from openfold.fused_adam_swa import FusedAdamSWA, kApexAdam, kPyTorchAdam
...
def test_fused_update_on_random_data():
    ...
    state_params = [torch.empty(random.randint(128, 2048), device=device, dtype=state_dtype).uniform_(-5, 5) for _ in range(32)]
    compute_dtypes = [compute_dtype if random.uniform(0., 1.) <= 0.5 else state_dtype for _ in range(32)]
    grads = [torch.empty_like(p, dtype=d).uniform_(-5, 5) for d, p in zip(compute_dtypes, state_params)]
    ...
    for i, p in enumerate(state_params): dummy_model.register_parameter(f"param_{i}", torch.nn.Parameter(p.clone()))
    ...
    optimizer = torch.optim.Adam(state_params_gt, lr=lr, betas=(beta1, beta2), eps=eps, weight_decay=weight_decay, amsgrad=amsgrad)
    ...
    for p, g in zip(state_params_gt, grads): p.grad = g.clone().to(state_dtype)
    optimizer.step()
    swa_model.averaged_model.n_averaged.copy_(swa_n_averaged)
    swa_model.update(dummy_model)
    for c, s in zip(compute_params_gt, state_params_gt): c.detach().copy_(s.detach().to(c.dtype))
    ...
    fused_optimizer = FusedAdamSWA(state_params_test, compute_params_test, swa_params_test, swa_decay_rate=swa_decay_rate, lr=lr, bias_correction=bias_correction, betas=(beta1, beta2), eps=eps, adam_math_mode=adam_math_mode, weight_decay=weight_decay, amsgrad=amsgrad)
    ...
    for c, g in zip(compute_params_test, grads): c.grad = g.clone()
    ...
    for i, (p_gt, p_test, p_origin) in enumerate(zip(state_params_gt, state_params_test, state_params)):
    ...
    assert swa_model.averaged_model.n_averaged.item() == fused_optimizer.swa_param_groups[0]["n_averaged"]
    for i, (p_test, p_gt) in enumerate(zip(
