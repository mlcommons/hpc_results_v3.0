...
class Optimizer(object):
  ...
  def apply_grads(self, grads, variables, aggregate_num = 999):
    ...
    """
    ops = []
    grads = mtf_ops.allreduce(grads, aggregate_num)
...
class MomentumOptimizer(Optimizer):
  ...
  def __init__(self, learning_rate, momentum, dense_weight_decay = 0.0):
    self._lr = learning_rate
  ...
  @property
  def dense_weight_decay(self):
    return self._dense_weight_decay
  ...
  def apply_grad(self, grad, var):
    ...
    if var.name.startswith("conv"):
      ...
      if "kernel" in var.name and "l2"in var.name:
          decay_rate = self.dense_weight_decay
    else:
