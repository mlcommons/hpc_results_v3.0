...
def conv3d_with_MPI(
  ...
  d_dim, h_dim, w_dim = x.shape.dims[-4:-1]
  for blocks_dim, block_size_dim, kernel_size in [
      (d_blocks_dim, d_dim, filter_size[0]),
      (h_blocks_dim, h_dim, filter_size[1])]:
    if kernel_size >= 2:
      if blocks_dim is not None:
        #x = mtf.halo_exchange_with_MPI(x, blocks_dim, block_size_dim, kernel_size)
        x = mtf.pad(x, [kernel_size // 2, kernel_size // 2], block_size_dim.name)
      else:
        x = mtf.pad(x, [kernel_size // 2, kernel_size // 2], block_size_dim.name)
