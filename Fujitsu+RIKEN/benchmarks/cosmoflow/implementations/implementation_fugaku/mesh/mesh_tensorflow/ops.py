...
def one_side_sendrecv_pyfunc(x,axis):
...
def one_side_sendrecv_backprop_pyfunc(x,axis):
    if num_active_size <= world_rank:
        return x
    ##logger.info("SendRecv_Pyfunc start")
    if axis == h_axis_idx:
...
def both_sides_sendrecv_pyfunc(left,right,axis):
    if num_active_size <= world_rank:
        return [left,right]
    s_reqs = []
    ##logger.info("SendRecv_Pyfunc start")
    if axis == h_axis_idx:
       ...
       right_peer = rank + 1
       if size == 1:
          left_halo = tf.zeros(shape=left.get_shape(),dtype=tf.float32)
          right_halo = tf.zeros(shape=right.get_shape(),dtype=tf.float32)
          return [left_halo, right_halo]
       #send
       if rank != 0:
          s_reqs.append(horizontal_comm.isend(left,dest=left_peer, tag=get_counter()))
       if rank != (size - 1):
          s_reqs.append(horizontal_comm.isend(right,dest=right_peer, tag=get_counter()))

       #recv
       if rank != 0:
          left_halo = horizontal_comm.recv(source=left_peer, tag=get_counter())
       else:
          left_halo = tf.zeros(shape=left.get_shape(),dtype=tf.float32)
       if rank != (size - 1):
          right_halo = horizontal_comm.recv(source=right_peer, tag=get_counter())
       else:
          right_halo = tf.zeros(shape=right.get_shape(),dtype=tf.float32)
       for req in s_reqs:
          req.wait()
       ...
       #send
       if rank != 0:
          s_reqs.append(vertical_comm.isend(left,dest=left_peer, tag=get_counter()))
       if rank != (size - 1):
          s_reqs.append(vertical_comm.isend(right,dest=right_peer, tag=get_counter()))

       #recv
       if rank != 0:
          left_halo = vertical_comm.recv(source=left_peer, tag=get_counter())
       else:
          left_halo = tf.zeros(shape=left.get_shape(),dtype=tf.float32)
       if rank != (size - 1):
          right_halo = vertical_comm.recv(source=right_peer, tag=get_counter())
       else:
          right_halo = tf.zeros(shape=right.get_shape(),dtype=tf.float32)
       for req in s_reqs:
          req.wait()
...
def tfone_side_sendrecv_backprop(dy,axis):
    dx = tf.py_function(func=one_side_sendrecv_backprop_pyfunc, inp=[dy,axis], Tout=tf.float32, name="one_side_sendrecv_backprop")
    dx.set_shape(dy.get_shape())
    return dx

def tfone_side_sendrecv(x,axis):
    y = tf.py_function(func=one_side_sendrecv_pyfunc, inp=[x,axis], Tout=tf.float32, name="one_side_sendrecv")
    y.set_shape(x.get_shape())
    return y

def tfboth_sides_sendrecv(x,axis):
    y = tf.py_function(func=both_sides_sendrecv_pyfunc, inp=[x[0],x[1],axis], Tout=[tf.float32]*2, name="both_sides_sendrecv")
    y[0].set_shape(x[0].get_shape())
    y[1].set_shape(x[1].get_shape())
    return tuple(y)
...
class OneSideSendRecvOperation(Operation):

  def __init__(self, x, sendrecv_dim_name, name=None):
    super(OneSideSendRecvOperation, self).__init__([x], name=name or "one_side_sendrecv")
  ...
  def gradient(self, grad_ys):
    dy = grad_ys[0]
    return [one_side_sendrecv_backprop(dy, self._axis)]
  ...
  def lower(self, lowering):
    ...
    def communication_fn(x, axis):
       ...
       x = tfone_side_sendrecv(x,axis)
       #logger.info("SENDRECV_TENSOR after %s" %(x))
       return x
    y = mesh_impl.slicewise(communication_fn, lowering.tensors[inputs], self._axis)
    lowering.set_tensor_lowering(self.outputs[0], y)

def one_side_sendrecv(x, sendrecv_dim_name, name=None):
   return OneSideSendRecvOperation(x, sendrecv_dim_name, name).outputs[0]

class OneSideSendRecvBackPropOperation(Operation):

  def __init__(self, x, axis, name=None):
    super(OneSideSendRecvBackPropOperation, self).__init__([x], name=name or "one_side_sendrecv_backprop")
    self._axis = axis
    self._outputs = [Tensor(self, x.shape, x.dtype)]
  ...
  def lower(self, lowering):
    ...
    def communication_fn(x, axis):
       ...
       x = tfone_side_sendrecv_backprop(x,axis)
       #logger.info("SENDRECV_BACKPROP_TENSOR after %s" %(x))
       return x
    dx = mesh_impl.slicewise(communication_fn, lowering.tensors[inputs], self._axis)
    lowering.set_tensor_lowering(self.outputs[0], dx)

def one_side_sendrecv_backprop(x, axis, name=None):
  return OneSideSendRecvBackPropOperation(x, axis, name).outputs[0]

class BothSidesSendRecvOperation(Operation):

  def __init__(self, xs, sendrecv_dim_name, name=None):
    super(BothSidesSendRecvOperation, self).__init__(xs, name=name or "both_sides_sendrecv")
  ...
  def gradient(self, grad_ys):
    dy = grad_ys
    return both_sides_sendrecv_backprop(dy, self._axis)

  def lower(self, lowering):
    mesh_impl = lowering.mesh_impl(self)
    def communication_fn(axis, size, *args):
       if size > 1:
          ys = tfboth_sides_sendrecv(args, axis)
          return ys
       return (args)
    values = mesh_impl.slicewise(communication_fn, self._axis, self._MPIsize, *[lowering.tensors[x] for x in self._inputs])
    for t, v in zip(self._outputs, values):
       lowering.set_tensor_lowering(t, v)

def both_sides_sendrecv(xs, sendrecv_dim_name, name=None):
   return BothSidesSendRecvOperation(xs, sendrecv_dim_name, name).outputs

class BothSidesSendRecvBackPropOperation(Operation):

  def __init__(self, ys, axis, name=None):
    super(BothSidesSendRecvBackPropOperation, self).__init__(ys, name=name or "one_side_sendrecv_backprop")
    output_tensors = [Tensor(self, y.shape, y.dtype) for y in ys]
    self._axis = axis
    if self._axis == d_axis_idx:
       self._MPIrank = vertical_comm.Get_rank()
       self._MPIsize = vertical_comm.Get_size()
  ...
  def lower(self, lowering):
    ...
    def communication_fn(axis, size, *args):
       if size > 1:
          xs = tfboth_sides_sendrecv(args,axis)
          return xs
       return (args)
   #kasagi
    values = mesh_impl.slicewise(communication_fn, self._axis, self._MPIsize,  *[lowering.tensors[y] for y in self._inputs])
    for t, v in zip(self._outputs, values):
       lowering.set_tensor_lowering(t, v)

def both_sides_sendrecv_backprop(ys, axis, name=None):
  return BothSidesSendRecvBackPropOperation(ys, axis, name).outputs
...
def allreduce_pyfunc(*x):
    ...
    if num_active_size <= world_rank:
       ...
       send_buf = np.zeros(sample_buf.size, dtype=x[0].dtype.as_numpy_dtype)
...
def cascade_allreduce_pyfunc(num, *ts):
    x = ts[:num]
    Tensors = []
    if num_active_size <= world_rank:
       np_list = [np.frombuffer(t, dtype=np.float32) for t in x]
       ...
       for i in range(len(x)):
           Tensors.append(tf.convert_to_tensor(np.reshape(np.frombuffer(recv_buf, dtype=np.float32, count=np_list[i].size, offset=s*4),x[i].shape),dtype=tf.float32))
           s += np_list[i].size
    return tuple(Tensors)+tuple(ts[num:])
...
def tfcascade_allreduce(idx, num, x):
    num_tensor = len(x[idx:])
    xl = list(x)
    y = tf.py_function(func=cascade_allreduce_pyfunc, inp=[num]+xl[idx:], Tout=[tf.float32]*num_tensor, name="cascade_allreduce")
    for i in xrange(num_tensor):
        y[i].set_shape(xl[idx+i].get_shape())
    return tuple(x[:idx])+tuple(y)
...
class CascadeAllreduceOperation(Operation):
  def __init__(self, xs, start_idx, num, name=None):
    super(CascadeAllreduceOperation, self).__init__(xs, name=name or "cascade_allreduce")
    output_tensors = [Tensor(self, x.shape, x.dtype) for x in xs]
    self._outputs = output_tensors
  ...
  def lower(self, lowering):
    mesh_impl = lowering.mesh_impl(self)
    def communication_fn(idx, num, *args):
       ys = tfcascade_allreduce(idx, num, args)
       return ys
    values = mesh_impl.slicewise(communication_fn, self._start_idx, self._num,  *[lowering.tensors[x] for x in self._inputs])
    for t, v in zip(self._outputs, values):
       lowering.set_tensor_lowering(t, v)

def allreduce(xs, aggregate_num = 1, name=None):
    n_tensors = len(xs)
    start = n_tensors - 1
    lx = xs
    while start != 0:
        num = aggregate_num
        if (start - aggregate_num) < 0:
            num = start
        start = start - num
        lx = CascadeAllreduceOperation(lx, start, num).outputs
    return lx
...
def halo_exchange_with_MPI(x, blocks_dim, block_size_dim, kernel_size, wrap=True):
  ...
  if partial_size > 0:
    if kernel_size == 2:
      left_margin = mtf_slice(x, 0, partial_size, block_size_dim.name)
      after_sendrecv = one_side_sendrecv(left_margin, block_size_dim.name)
      parts = (parts + [after_sendrecv])
    else:
      left_margin = mtf_slice(x, 0, partial_size, block_size_dim.name)
      right_margin = mtf_slice(
          x, block_size_dim.size - partial_size, partial_size,
          block_size_dim.name)
      after_sendrecv = both_sides_sendrecv([left_margin,right_margin], block_size_dim.name)
      parts = ([after_sendrecv[0]] + parts + [after_sendrecv[1]])
